...
Use this skill when the target journal is clearly `Foods`. Treat it as a shortcut into the MDPI workflow with the target journal preselected.

Prefer the current Foods instructions page or the user's uploaded template over any bundled notes.

This skill should support minimal prompts plus a direct manuscript upload.

## Workflow

1. Confirm the article type for Foods.
2. Read the current Foods instructions page in `references/official-guidelines.md` or a newer user-provided guideline URL.
3. Apply the `formatting-mdpi` workflow and statement checks, but treat Foods-specific journal instructions as final when they differ from general MDPI patterns.
...
- Foods instructions URL or template, only if already available or needed for stricter template matching.
